...
class KSHZoom {
	...
	applyEdits() {
		ZOOM_MODS.forEach((MOD_NAME) => {
			...
			if(DEBUG){
				// Debug print
				console.log("Type", MOD_NAME);
				console.log("Base:");
				console.log(mod_arr.map((x) => x.toString()).join('\n'));
				console.log("Edit:");
				console.log(edit_arr.map((x) => x.toString()).join('\n'));
				console.log("Patch generated:");
				console.log(patches.map((x) => x.toString()).join('\n'));
			}
			...
			if(DEBUG) {
				console.log("After:");
				console.log(mod_arr.map((x) => x.toString()).join('\n'));
				if(mod_arr.length > 0 && mod_arr[mod_arr.length-1].afterValue()) throw new Error();
			}
