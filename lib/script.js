			...
			if(line_arr[0] == ';'){
				switch(line_arr[1]){
					...
					case 'import':
						...
						const script = KLEScript.fromFilePath(line_arr.slice(2).join(' '));
...
KLEScript.fromFilePath = (p) => {
	if(p == 'stdlib') return SCRIPT_CACHE['stdlib'];
	let ap = path.resolve(process.cwd(), p);
	if(ap in SCRIPT_CACHE) return SCRIPT_CACHE[ap];
	return SCRIPT_CACHE[ap] = new KLEScript(fs.readFileSync(ap, 'utf-8'));
};

SCRIPT_CACHE['stdlib'] = new KLEScript(fs.readFileSync(path.resolve(__dirname, "stdlib.kle"), 'utf-8'));
