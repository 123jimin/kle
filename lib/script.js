...
const INVALID_COMMAND_NAMES = new Set(['define', 'command', 'end', 'set', 'import', 'include']);
...
class KLECommand {
	constructor(lines) {
		if(lines.length == 0){
			throw new Error(`Empty command definition!`);
		}
	...
	calc(context, eq) {
		...
		if(eq.match(/[^0-9().+\-*/]/))
			throw new Error(`Invalid expression (${orig_eq}) for command "${this.name}"!`);
